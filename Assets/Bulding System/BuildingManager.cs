...
public class BuildingManager : MonoBehaviour
{
    ...
    // True when a build slot is selected (not axe, not nothing)
    ...
    private void Start()
    {
        ...
        // Start with build mode OFF (no item selected)
    ...
    {
        for (int i = 0; i < Mathf.Min(hotbarSlots.Count, 9); i++)
        {
        ...
        if (scroll != 0f)
        {
            ...
            int newIndex = (selectedHotbarIndex + dir + hotbarSlots.Count) % hotbarSlots.Count;
    ...
    public void SelectHotbarSlot(int index)
    {
        ...
        buildModeActive = true;   // selecting a build item activates build mode
    ...
    // Called by HotbarUI when the axe (E) is selected — disables build mode
